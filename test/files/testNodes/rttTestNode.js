const { ClientNodeZMQ } = require('./clientNodeZMQ.js');
const { defaultServiceServerPort } = require('../../../src/node/constants');
const { ZmqDealer } = require('@tum-far/ubii-msg-transport');
...
class RttTestNode extends ClientNodeZMQ {
  constructor(
    ...
    servicePort = defaultServiceServerPort,
    topic,
    onFinishedCallback
  ) {
    ...
    super(
      ...
      message => {
        ...
        let repTime = JSON.parse(
          `[ ${message.topicDataRecord[message.topicDataRecord.type].toString()}]`
        );
        let results = [];
        ...
        if (currentIteration === numberOfIterations) {
          ...
          namida.logSuccess(
            'Local dealer/router round-trip time test result',
  ...
  async startRttMeasurement() {
    ...
    namida.log(
      'Local dealer/router round-trip time test started',
